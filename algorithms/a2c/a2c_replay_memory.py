...
class A2CReplayMemory:
    """
    Replay memory for the A2C algorithm. This algorithm remembers tuples:
        (state, action, reward, next_state, done)

    These are collected for a specific number of episodes, which are all
    returned when sampled. A2C is an on-policy algorithm, so it needs to
